...
const TodoInput = ({ theme, title, description, setTitle, setDescription, handleAdd, editId }: TodoInputProps) => {

    return (
        <div className={styles.form}>
            <input className={`${styles.input} ${theme ? styles.light : styles.dark}`} onChange={(e: ChangeEvent<HTMLInputElement>) => setTitle(e.target.value)} name='title' type="text" placeholder='Enter Task Title...' value={title} />
            <textarea className={`${styles.textarea} ${theme ? styles.light : styles.dark}`} onChange={(e: ChangeEvent<HTMLTextAreaElement>) => setDescription(e.target.value)} name='description' placeholder='Enter Task Description...' value={description} />
            <button className={`${styles.addbutton} ${theme ? styles.buttonlight : styles.buttondark}`} onClick={handleAdd}>
                <div className={`${styles.addcontainer} ${theme ? styles.buttonlight : styles.buttondark}`}>
                    {editId !== null ?
